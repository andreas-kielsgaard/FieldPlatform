...
3. Use human-maintained maps, source reads, and `rg` when repository evidence is needed for generated markers, producer scripts, tool contracts, freshness, or direct-edit safety. Legacy `artifact-query` is inactive unless the human explicitly asks for legacy Agent OS index/tool maintenance.
4. Find the producing source, tool contract, map, or instruction surface when available.
5. Use source reads and consumer mapping when generated or indexed output has consumers that may rely on shape, freshness, or compatibility; legacy tool-backed preview notes are inactive by default.
...
Update `prompt-files/agent-attention-system/maps/tool-map.md` or tool semantic files only when an explicit Agent OS maintenance task changes legacy tool routing or execution expectations.
