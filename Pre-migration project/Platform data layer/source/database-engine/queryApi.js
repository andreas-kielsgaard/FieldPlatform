const FieldPlatformQueryApi = (() => {
  function createQueryApi(database) {
    ...
    function canManageEvent(personId, eventId) {
      ...
      if (event.creatorId === personId || event.hostId === personId) return true;
      if ((event.cohostIds || []).includes(personId)) return true;
      if ((event.volunteerIds || []).includes(personId)) return true;
    ...
    function canManageCommunity(personId, groupId) {
      ...
      if (group.creatorId === personId || (group.stewards || []).includes(personId)) return true;
    ...
    return {
      ...
      getFieldRelationsBetween: (sourceType, sourceId, targetType, targetId) => list("fieldRelations").filter(relation =>
        relation.sourceType === sourceType &&
        relation.sourceId === sourceId &&
        relation.targetType === targetType &&
        relation.targetId === targetId
      ),
      getFieldRelationsForReviewAuthority: (authorityType, authorityId) => list("fieldRelations").filter(relation =>
        relation.reviewAuthorityType === authorityType &&
        relation.reviewAuthorityId === authorityId
      ),
      getPendingFieldRelationsForReviewAuthority: (authorityType, authorityId) => list("fieldRelations").filter(relation =>
        relation.reviewAuthorityType === authorityType &&
        relation.reviewAuthorityId === authorityId &&
      ...
      getDataShareRequestsForSubject: (subjectType, subjectId) => list("dataShareRequests").filter(request =>
        request.subjectType === subjectType &&
        request.subjectId === subjectId
      ),
      getDataShareRequestsForContext: (contextType, contextId) => list("dataShareRequests").filter(request =>
      ...
      getVisibilityGrantsForSubject: (subjectType, subjectId) => list("visibilityGrants").filter(grant =>
        grant.subjectType === subjectType &&
        grant.subjectId === subjectId
      ),
      getVisibilityGrantsForContext: (contextType, contextId) => list("visibilityGrants").filter(grant =>
        grant.contextType === contextType &&
