# Tool: Update State Management Index

Tool ID: `update-state-management-index`

Script: `tools/agent/index-access/update-state-management-index.ts`

## Purpose

Refresh generated `docs/agent/generated-indexes/state-management-map.md` data for state owners, stores, consumers, update paths, derived state, URL state, and cache state, or report that refresh is unavailable.

## Expected Invocation

```powershell
npx tsx tools/agent/index-access/update-state-management-index.ts
```
