...
1. Current [user](../Data%20layer/Person%20entity.md) instruction.
2. Current `General context/` wiki pages.
3. Platform [data layer](../Architecture/Data%20layer%20overview.md) docs and implementation for current interface truth.
...
Update context when durable product grammar, language, data meaning, [view](../Frontend/Views%20overview.md) behavior, or process guidance changes.

Update the Platform [data layer](../Architecture/Data%20layer%20overview.md) when multiple surfaces need the same entity, relation, calculation, access method, permission, or lifecycle behavior.

Build or revise a mockup when the question is about feel, hierarchy, comprehension, or flow.

Do not build yet when the [concept](../Ontology/Product%20ontology.md) is still philosophical, the input burden is not justified, the platform would need to pretend it knows too much, or proximity/direct action already solves the need better.
...
General context changes require explicit [user](../Data%20layer/Person%20entity.md) request or review before commit, merge, or push. This wiki migration was explicitly requested as a context change and should still be reported clearly.
