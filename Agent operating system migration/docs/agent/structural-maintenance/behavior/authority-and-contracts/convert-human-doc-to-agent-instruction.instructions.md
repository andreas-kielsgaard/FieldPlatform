...
3. Use `audience-surface-check` when the intended audience or loading surface is unclear.
4. Use `authority-surface-search` when the same rule may already exist in an agent-facing surface.
...
- Route into a task-mode, behavior, map, checklist, or tool instruction.
...
Update the deferred logging strategy when the conversion establishes a durable documentation or instruction convention.
