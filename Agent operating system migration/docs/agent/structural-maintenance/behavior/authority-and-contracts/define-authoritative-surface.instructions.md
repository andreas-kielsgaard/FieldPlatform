...
3. Use `authority-surface-search` when a bounded repository search is needed to find duplicate, conflicting, explanatory, generated, or stale authority candidates.
4. Use `audience-surface-check` when the likely authority surface and intended audience may not match.
