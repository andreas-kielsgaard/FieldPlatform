...
3. Use `artifact-maintenance-path` when repository evidence is needed for generated markers, producer scripts, tool contracts, freshness, or direct-edit safety.
4. Find the producing source, tool contract, map, or instruction surface when available.
5. Use `contract-impact` when generated or indexed output has consumers that may rely on shape, freshness, or compatibility.
...
Update `docs/agent/tools/tooling-map.md` or tool instruction files when the maintenance path changes tool routing or execution expectations.
