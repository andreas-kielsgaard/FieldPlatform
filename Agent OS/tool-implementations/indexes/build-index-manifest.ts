...
const definition: IndexDefinition = {
  ...
  sourceInputs: ["legacy Stratum 1 index artifacts, retired index tier policy, and historical query guidance"],
  coverage: "Legacy metadata for retired evidence indexes, including tier, freshness, size, hash, and historical first-query guidance.",
  knownBlindSpots: [
  ...
  return ACTIVE_INDEX_CATALOG.map((entry) => {
    if (entry.indexId === "index-manifest") {
      return {
        ...entry,
        stratum: 1,
        bootloaderRole: "legacy only; not mandatory boot context",
        loadPolicy: "Do not read this artifact during ordinary Agent OS initiation; use only for explicit legacy index maintenance.",
        exists: true,
        byteSize: null,
        sha256: null,
        schemaVersion: 2,
        ...
        freshnessPolicy: "Refresh only during explicit legacy Agent OS index maintenance.",
    ...
    return {
      ...entry,
      stratum: 1,
      bootloaderRole: entry.tier === "boot-core" ? "legacy boot-core cue" : "legacy selective cue",
      loadPolicy: "Legacy row only; do not query raw records during ordinary development.",
      exists,
