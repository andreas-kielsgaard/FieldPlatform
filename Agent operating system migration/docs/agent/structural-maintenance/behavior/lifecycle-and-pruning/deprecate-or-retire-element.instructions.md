...
3. Use `contract-impact` and `contract-test-coverage` when deprecation or retirement changes supported behavior or compatibility.
4. Use `artifact-maintenance-path` when generated, indexed, or tool-maintained status affects the retirement path.
...
Update the deferred logging strategy when retirement or deprecation changes durable architecture.
