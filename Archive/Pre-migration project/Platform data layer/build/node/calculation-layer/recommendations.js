...
const FieldPlatformRecommendationCalculations = (() => {
    ...
    function recommendEventsForParticipant(data, personId) {
        ...
        return data.events
            ...
            const reasons = [];
            if (linkedSignal)
                reasons.push("event is adjacent to communities you already participate in");
            if (tagSignal)
                reasons.push(`${tagSignal} tag matches from your current orientation`);
            if (lowThreshold)
                reasons.push("low-threshold entry point");
            if (event.access !== "public")
                reasons.push("visible, but signup has a stronger commitment layer");
    ...
    function recommendGroupsForEvent(data, draftEvent) {
        ...
        return data.groups
            ...
            const score = clamp(tagOverlap.length * 18 +
                venueFit * 16 +
            ...
            if (beginnerFit)
                reasons.push("good low-threshold entry fit");
    ...
    function computePersonalGroupMetrics(data, personId, groupId) {
        ...
        const participatedEvents = data.events.filter(event => event.attendance.attending.includes(personId) || event.attendance.interested.includes(personId));
    ...
    function computeCreatorGroupSignal(data, creatorId, groupId, draftEvent = null) {
        ...
        const draftTags = (draftEvent === null || draftEvent === void 0 ? void 0 : draftEvent.tags) || [];
        ...
        const relevantCreatorEvents = creatorEvents.filter(event => event.linkedGroups.includes(groupId) ||
            event.relevantGroups.includes(groupId));
        const markedRelevantShares = data.suggestedEventShares.filter(share => share.groupId === groupId &&
