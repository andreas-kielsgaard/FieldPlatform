...
6. Use maps, ledgers, tool instructions, generated-index slices, and full lens files only when selected behavior files require deeper evidence, memory updates, or clarification.
7. Resolve logical tool IDs through `docs/agent/tools/tooling-map.md`.
...
Generated indexes under `docs/agent/generated-indexes/` should be accessed as bounded evidence surfaces, normally through tools or narrow slices, not full prompt ingestion.
