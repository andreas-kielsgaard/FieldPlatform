...
- Public API, exports, types, schemas, accessors, tool instruction files, fixtures, examples, tests, and consumers.
- Unit tests and contract tests covering changed internal processing or I/O relations.
- Tool evidence from `contract-impact` and `contract-test-coverage` when consumer or coverage evidence needs to be bounded.
