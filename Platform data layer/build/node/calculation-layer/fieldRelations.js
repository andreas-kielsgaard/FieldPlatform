...
const FieldPlatformFieldRelationCalculations = (() => {
    ...
    function movementOptionsForRelation(snapshot, relationId) {
        ...
        if (relation.status === "declined")
            return ["remain_observing"];
        if (relation.status === "suggested")
            return unique([...(relation.movementUnlocked || []), "ask_steward", "remain_observing"]);
        if (relation.visibility === "private" && relation.sourceType !== "person")
            return unique(relation.movementUnlocked || []);
        const derived = [];
        if (relation.status === "accepted" || relation.status === "refined" || relation.status === "computed") {
            if (relation.relationKind === "good_first_step_for" || relation.relationKind === "belongs_with") {
                if (relation.sourceType === "event" || relation.targetType === "event")
                    derived.push("attend", "mark_interested");
                if (relation.sourceType === "community" || relation.targetType === "community")
                    derived.push("follow", "request_access");
            }
            if (relation.relationKind === "soft_landing_after")
                derived.push("attend", "follow");
            if (relation.relationKind === "bridges_to")
                derived.push("follow", "suggest_connection");
            if (relation.sourceType === "person" || relation.targetType === "person")
                derived.push("remain_observing", "request_access", "ask_steward");
        }
    ...
    function describeEndpoint(snapshot, objectType, objectId) {
        ...
        return {
            objectType,
            objectId,
            label: (record === null || record === void 0 ? void 0 : record.name) || (record === null || record === void 0 ? void 0 : record.title) || (record === null || record === void 0 ? void 0 : record.id) || objectId
        };
    }
    function getRecord(snapshot, objectType, objectId) {
        if (objectType === "person")
            return shared.getPerson(snapshot, objectId);
        if (objectType === "community")
            return shared.getGroup(snapshot, objectId);
        if (objectType === "event")
            return (snapshot.events || []).find(event => event.id === objectId) || (snapshot.createdEvents || []).find(event => event.id === objectId);
        if (objectType === "venue")
            return shared.getVenue(snapshot, objectId);
        if (objectType === "festival")
            return (snapshot.festivals || []).find(festival => festival.id === objectId);
        if (objectType === "generatedField")
            return { id: objectId, name: objectId };
    ...
    function fallbackReason(relation) {
        if (relation.status === "suggested")
            return "Suggested connection waiting for review.";
        if (relation.status === "computed")
            return "Calculated from shared context.";
