...
- `AGENTS.md`, task-mode files, structural-maintenance files, maps, ledgers, tool instruction files, tests, schemas, README files, and migration notes.
- Existing references to the rule or concept.
- Tool evidence from `authority-surface-search` when rule ownership or duplicated authority needs a bounded search.
- Tool evidence from `artifact-maintenance-path` when generated or tool-maintained artifacts appear to report authority.
- Prior decisions in the deferred logging strategy.
